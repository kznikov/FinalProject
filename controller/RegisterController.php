...
if (isset($_POST['submit'])) {
    ...
    if (!empty($firstname) && $userData->checkUserName($username) && $userData->checkEmail($email) && !empty($lastname) && !empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL) && !empty($username) && !empty($password) && $password === $repassword && UserDAO::checkEmail($email) && UserDAO::checkUserName($username)) {
        try {
